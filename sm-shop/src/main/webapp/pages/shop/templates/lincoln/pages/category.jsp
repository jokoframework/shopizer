...
<script>
	...
	function loadCategoryProducts() {
		var categoryUrl = categoryFilter != undefined ? categoryFilter : '<c:out value="${category.description.friendlyUrl}"/>';
		var url = '<%=request.getContextPath()%>/services/public/products/page/' + START_COUNT_PRODUCTS + '/' + MAX_PRODUCTS + '/<c:out value="${requestScope.MERCHANT_STORE.code}"/>/<c:out value="${requestScope.LANGUAGE.code}"/>/'+ categoryUrl;
...
<%--		<p>--%>
<%--		<c:choose>--%>
<%--		<c:when test="${categoryFilter!=null}">--%>
<%--					<c:out value="${categoryFilter}"/>--%>
<%--		</c:when>--%>
<%--		<c:otherwise>--%>
<%--			<c:out value="${category.description.description}"/>--%>
<%--		</c:otherwise>--%>
...
<div class="row-fluid">
	...
	<div class="span12">
		...
		<div class="span3">
			<div class="sidebar-nav">
				<ul class="nav nav-list">
					...
					<li class="nav-header"><s:message code="label.manufacturer.categories" text="Categories" /></li>
					<c:forEach items="${requestScope.TOP_CATEGORIES}" var="category">
						<li class="">
							<a href="javascript:filterByCategory('BRAND','${manufacturer.id}','${category.description.friendlyUrl}','${category.description.name}')"><c:out value="${category.description.name}" /></a></li>
		...
		<!-- right column -->
		<div class="span9">
			<p id="CategoryTitle" class="lead"><c:out value="${category.description.name}" /></p>
